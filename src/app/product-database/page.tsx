...
export default function ProductDatabase() {
  ...
  const handleSearch = () => {
    setAppliedSearchTerm(searchTerm); // Apply search term to trigger API call
    setCurrentPage(1); // Reset to first page on search
  };

  // Fetch products on initial load and when page or search term changes
  useEffect(() => {
  ...
  const fetchProducts = async (page: number, search = "") => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/product/product-database?page=${page}&limit=${itemsPerPage}&search=${encodeURIComponent(search)}`,
        {
          headers: {
      ...
      const total = data?.data?.total || 0; // Assuming API returns total items
